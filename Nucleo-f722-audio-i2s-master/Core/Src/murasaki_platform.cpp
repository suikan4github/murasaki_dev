...
void InitPlatform()
{
    ...
    murasaki::platform.led1 = new murasaki::BitOut(LD1_GPIO_Port, LD1_Pin);
...
void HAL_I2S_RxHalfCpltCallback(I2S_HandleTypeDef *hi2s) {
    if (murasaki::platform.audio->DmaCallback(hi2s, 0)) {
        murasaki::platform.led1->Set();
        murasaki::platform.led2->Clear();
        return;
    }
}
...
void HAL_I2S_RxCpltCallback(I2S_HandleTypeDef *hi2s) {
    if (murasaki::platform.audio->DmaCallback(hi2s, 1)) {
        murasaki::platform.led1->Clear();
        murasaki::platform.led2->Set();
        return;
    }
}
